...
rb_red_blk_tree* RBTreeCreate( int (*CompFunc) (const void*,const void*),
  ...
  memset(newTree->nil,0,sizeof(rb_red_blk_node));
  temp->parent=temp->left=temp->right=temp;
  temp->red=0;
  temp->key=0;
  temp=newTree->root= (rb_red_blk_node*) SafeMalloc(sizeof(rb_red_blk_node));
  memset(newTree->root,0,sizeof(rb_red_blk_node));
...
rb_red_blk_node* RBExactQuery(rb_red_blk_tree* tree, void* q) {
  ...
  int compVal;
  if (x == nil) return(0);
  compVal=tree->Compare(x->key,(void*) q);
...
stk_stack * RBEnumerateNodes(rb_red_blk_node* x,rb_red_blk_node* nil,stk_stack* enumResultStack)
{
  if (x != nil) 
  {
	RBEnumerateNodes(x->right,nil,enumResultStack);
	StackPush(enumResultStack,x);
	RBEnumerateNodes(x->left,nil,enumResultStack);
    
  }
  return(enumResultStack);
}

stk_stack * RBGetAllNodes(rb_red_blk_tree* tree)
{
	stk_stack* enumResultStack=StackCreate();
	if(enumResultStack)
	{
		enumResultStack=RBEnumerateNodes(tree->root,tree->nil,enumResultStack);
		return enumResultStack;
	}
	return NULL;
}
